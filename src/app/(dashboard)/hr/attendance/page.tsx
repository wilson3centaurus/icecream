  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {shiftSummary.map((s) => (
          <div key={s.label} className="rounded-2xl border border-border bg-white p-5 dark:border-white/8 dark:bg-white/5">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-brown dark:text-white">{s.label} Shift</p>
              <span className="text-xs text-muted dark:text-white/40">{s.present}/{s.total}</span>
            </div>
            <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-gray-200 dark:bg-white/8">
              <div className={`h-full rounded-full ${s.color}`} style={{ width: `${(s.present / s.total) * 100}%` }} />
            </div>
            <p className="mt-2 text-2xl font-bold text-brown dark:text-white">{s.present} <span className="text-sm font-normal text-muted dark:text-white/40">present</span></p>
          </div>
        ))}
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-white dark:border-white/8 dark:bg-white/5">
        <div className="border-b border-border px-5 py-3.5 flex items-center gap-2 dark:border-white/8">
          <Clock className="h-4 w-4 text-orange" />
          <span className="text-sm font-medium text-brown dark:text-white">Today&apos;s Records</span>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border dark:border-white/8">
              ...
              <th className="px-5 py-3.5 text-center text-xs font-medium text-muted dark:text-white/40">Clock In</th>
              <th className="px-5 py-3.5 text-center text-xs font-medium text-muted dark:text-white/40">Clock Out</th>
              <th className="px-5 py-3.5 text-center text-xs font-medium text-muted dark:text-white/40">Hours</th>
              <th className="px-5 py-3.5 text-center text-xs font-medium text-muted dark:text-white/40">OT</th>
              <th className="px-5 py-3.5 text-left text-xs font-medium text-muted dark:text-white/40">Status</th>
            </tr>
          </thead>
          <tbody>
            {records.map((r) => (
              <tr key={r.name} className="border-b border-border transition hover:bg-gray-50 last:border-0 dark:border-white/5 dark:hover:bg-white/4">
                <td className="px-5 py-3.5">
                  <p className="font-medium text-brown dark:text-white">{r.name}</p>
                  <p className="text-[11px] text-muted/70 dark:text-white/30">{r.dept}</p>
                </td>
                <td className="px-5 py-3.5 text-xs text-muted dark:text-white/50">{r.shift}</td>
                <td className="px-5 py-3.5 text-center font-mono text-xs text-brown dark:text-white/70">{r.in}</td>
                <td className="px-5 py-3.5 text-center font-mono text-xs text-muted dark:text-white/50">{r.out}</td>
                <td className="px-5 py-3.5 text-center text-xs text-muted dark:text-white/60">{r.hours}</td>
